...
<div class="sheet-wrap"><div class="sheet-caption">本章预览</div>


- 我们首先介绍函子时，我们看到它们对于可以被映射于的值来说，是一个很有用的概念
- 然后，我们通过介绍应用函子，把概念进了一步，它允许我们把特定数据类型的值看作具有上下文的值，并且使用将普通的函数使用于这些值的同时，还保留这些上下文的含义
- 在本章中，我们将会学习单子（monad），它只是增强版的应用函子，就像应用函子只是增强版的函子

- 当我们从函子开始时，我们看到有可能把函数映射于不同的数据类型
- 我们看到为此，引入了Functor类型类，它让我们问这个问题：
  - 当我们拥有一个类型`a -> b`的函数，以及一些类型`f a`的数据
  - 我们如何把函数映射于数据类型，最终是`f b`？
- 为了解答这个如何将函数应用于某些数据类型的问题，我们必须看fmap的类型
  ``` Haskell
  fmap :: (Functor f) => (a -> b) -> f a -> f b
  ```
- 然后通过编写合适的Functor示例，它就给我们打工了

- 我们看到函子可能的提升，并且说，要如果那个函数`a -> b`已经包裹在函子值里了呢？
  - 比如说，要如果我们有`Just (*3)`，我们如何把它应用到`Just 5`呢？
  - 如果我们不想把它应用到`Just 5`，而是Nothing呢？
  - 或者如果我们有`[(*2), (+4)]`，我们如何把它应用到`[1,2,3]`呢？
  - 它甚至如何运作？
- 为此，引入了Applicative类型类，其中我们想要下列类型的答案：
  ``` Haskell
  (<*>) :: (Applicative f) => f (a -> b) -> f a -> f b
  ```
- 我们也看到我们可以获取一个值并且把它包裹在一个数据类型中
  - 例如，我们可以获取1并且包裹它，因此它变成了`Just 1`
  - 或者我们可以让它变成`[1]`
  - 或者一个I/O行为，它什么都不做，只是产生1
  - 完成这个的函数被称为pure
- 就像我们说的，一个应用值被看作一个具有额外上下文的值
- 用严格的术语来说（to put it in technical terms），是一个奇特的值（a fancy value）
  - 例如说，字符'a'只是一个普通的字符，而`Just 'a'`具有一些额外的上下文
  - 并非一个Char，我们有`Maybe Char`，它告诉我们它的值可能是一个字符，但是它也可以没有一个字符
- 看Applicative类型类如何允许我们对这些带有上下文的值使用普通的函数，以及上下文如何保留，是很漂亮的事情。看：
  ``` Haskell
  ghci> (*) <$> Just 2 <*> Just 8
  Just 16
...
- 既然我们把它们当作应用值
  - `Maybe a`值代表可能会失败的计算
  - `[a]`值代表具有数个值的计算（非确定性计算）
  - `IO a`值代表具有副作用的值，等等

Monad是应用函子的自然延伸，通过它们，我们关注这个
- 如果你有一个带上下文的值，`m a`，你如何把一个获取一个普通`a`并且返回带有上下文的值的函数，应用于它？
- 也就是说，你如何应用一个类型`a -> m b`于类型`m a`的值？
- 所以本质上，我们想要这个函数
  ``` Haskell
  (>>=) :: (Monad m) => m a -> (a -> m b) -> m b
  ```
- 如果我们有奇异的值，以及一个函数，其获取普通值但是返回奇异的值，我们如何把那个奇异的值喂进函数？
- 这就是我们处理单子时将会关心的主要问题
- 我们写`m`而不是`f`，因为`m`代表Monad，但是单子只是支持`>=`的应用函子
- `>>=`度做绑定（bind）

- 当我们有一个普通值`a`和一个普通函数`a -> b`，把值喂到函数里面很简单——你只需要把函数正常应用于值，就行了
- 但是当我们处理带有特定上下文的值，它得花点功夫思考，看看这些奇异的值如何喂到汉书礼，以及如何考虑它们的行为
- 但是你会看到它就跟数一二三一样简单
...
- 目前为止，我们已经看到
  - Maybe值如何能够被看作带有失败上下文的值
  - 我们如何能够把错误处理合并到我们的代码中，通过使用>>=将它们喂入函数
- 本节，我们将会看看如何利用列表单子的一面，把非确定性以一种干净和可读的方式带入我们的代码

</div>
<div class="sheet-wrap"><div class="sheet-caption">列表作为应用函子与不确定性计算</div>

</div>
<div class="sheet-wrap"><div class="sheet-caption">列表作为Monad实例</div>


让我们看看列表作为Monad实例看起来如何
``` Haskell
instance Monad [] where
  ...
  fail _ = []
```
- return和pure做的事情一样，所以我们应该已经对列表的return很熟悉了
  - *略*
  - 当我们想要把一个普通的值包裹进列表，让它能够和非确定性值互动，这是很有用的
- 要理解>>=如何对列表其作用，最好是我们动手看看，来首先获取一些直觉
  - >>=是关于从一个上下文（单子值）中获取一个值，将其喂入获取普通值、返回有上下文的值的函数
  - 如果那个函数只是产生了一个普通值，而不是上下文，>>=不会很有用，因为在一次使用过后，上下文就被失掉了
- 让我们尝试把非确定性值喂入一个函数
  ``` Haskell
  ghci> [3,4,5] >>= \x -> [x,-x]
  [3,-3,4,-4,5,-5]
  ```
  - 当我们对Maybe使用>>=，单子值喂入函数，并且关注可能的失败
  - 这里，它帮我们关注非确定性
  - `[3,4,5]`是非确定性值，我们把它喂入一个函数，它也返回非确定性值，结果也是非确定性的
- 为了看这是如何实现的，我们只要跟着实现
  1. 我们从列表[3,4,5]开始
  2. 我们把lambda映射于它，结果是
     ``` Haskell
     [[3,-3],[4,-4],[5,-5]]
     ```
  3. lambda被应用于每一个元素，我们获得一个列表的列表
  4. 最终，我们只要展平列表
- 非确定性计算也包括失败支持
  - 空列表[]就很大程度上等于Nothing，因为它代表没有结果
  - 这也就是为什么fail通常被定义为空列表
- 让我们玩玩失败的列表 \
  *代码略*
- 正如用Maybe值，我们可以用>>=链接若干个列表，体现非确定性（propagating the non-determinism）
  ``` Haskell
  ghci> [1,2] >>= \n -> ['a','b'] >>= \ch -> return (n,ch)
  [(1,'a'),(1,'b'),(2,'a'),(2,'b')]
  -- 这里的第1个lambda表达式里面嵌套了第2个lambda表达式
  ```

注意：当你在做非确定性值互动时，你可以把它们的计算看作一个树，列表中每一个可能的值代表一个单独的分支

之前的表达式用do记号重写
``` Haskell
lispOfTuples :: [(Int, Char)]
listOfTuples = do
  n <- [1,2]
  ch <- ['a','b']
  return (n,ch)
```
- 这看着就有点更加明显，n从[1,2]中获取每一个值，ch从['a','b']中获取每一个值
- 这种情况的上下文是非确定性的

</div>
<div class="sheet-wrap"><div class="sheet-caption">回忆列表推导式</div>


在do记号中使用列表让我想起我们已经见过的东西，看看下列代码
``` Haskell
...
- 在我们的do记号示例中
  - n变成了每一个来自[1,2]的结果
  - 对于每个这样的结果，ch被赋为来自['a','b']的结果
  - 最后一行把(n,ch)放进默认上下文，来把它表示为结果，并且不引入任何额外的非确定性
  - 这个列表推导式中，同样的事情发生了，只是我们不需要在最后写return来把(n,ch)作为结果表示，因为列表推导式的输出部分帮我们做了
- 实际上，列表推导式只是把列表作为单子的语法糖（syntactic sugar）
- 最终，列表推导式和do记号里的列表翻译为使用>>=来做以非确定性为特征的运算

</div>
<div class="sheet-wrap"><div class="sheet-caption">列表推导式的滤出如何翻译为单子</div>


列表推导式允许我们滤出（filter）我们的结果
- 例如，我们可以过滤一个数字列表来只搜索数位包含7的数字
  ``` Haskell
  ghci> [x | x <- [1..50], '7' `elem` show x]
  [7,17,27,37,47]
  ```
- 我们把show应用于x来把数字转换成字符串，然后我们检查字符'7'是否在字符串中，很聪明
- 为了看列表推导式中的过滤如何翻译成列表单子，我们必须检查为函数以及MonadPlus类型类
- MonadPlus类型类是针对也可以作为幺半群使用的单子的，定义为：
  ``` Haskell
  class Monad m => MonadPlus m where
    mzero :: m a
    mplus :: m a -> m a -> m a
  ```
  - `mzero`，是Monoid类型类里面`mempty`的同义词
  - `mplus`对应于`mappend`
- 因为列表既是幺半群又是单子，它们可以作为这个类型类的实例
  ``` Haskell
  instance MonadPlus [] where
    mzero = []
    mplus = (++)
  ```
  - 对于列表`mzero`代表没有任何结果的非确定性计算——一个失败的计算
  - `mplus`把两个非确定性值结合成一个
  - `gaurd`函数像这样定义
    ``` Haskell
  ...
  - 这就是测试
    ...
    ```
  - 看起来有趣，但是这是怎么有用的呢
  - 在列表单子中，我们用它来滤除非确定性计算，看
    ``` Haskell
    ghci> [1..50] >>= return "cool" :: [String]
    [7, 17, 27, 37, 47]
    ```
  - 这里的结果和我们之前列表推导式的结果一样，这是怎么做到的？
    - 让我们首先看看卫函数是如何用>>结合的
      ``` Haskell
      ...
      []
      ```
    - 如果guard成功了，结果里面包含了一个空元组，所以，我们使用>>来忽略那个空元组，然后把另外的东西呈现作为结果
    - 然而，如果guard失败了，这个就会随即返回，因为往带有>>=的函数中喂入空列表结果总是空列表
    - guard基本上是在说
      - 如果布尔值为假，那么就在这里产生失败
      - 否则产生一个成功值，里面有一个无用的结果()
...
sevenOnly = do
  ...
  return x
```
- 加入我们忘记把x用return表达为最终结果，结果列表就只有一列空元组
- 所以列表推导式中的过滤也是使用了guard
...
我们已经看到
- 单子如何用于把值放到上下文，然后把它们应用到函数
- 以及使用`>>=`或者do记号如何允许我们集中注意力于值本身，同时上下文也帮我们处理好了
- 我们遇到了Maybe单子并且看到了它如何给值增加了一个可能失败的上下文
- 我们学习了list单子以及看到了它如何让我们很容易地往我们的程序引入不确定性
- 我们也学习了如何左IO单子中工作，即使我们还没知道单子是什么！

本章
- 我们将会学习其它几个单子
- 我们将会看到它们如何通过让我们把所有的值都当作单子值，来让我们的程序更加清晰
- 进一步探索几个单子将也会巩固我们对单子的直觉

我们将会探索的单子是mtl包的一部分
- 一个Haskell包就是一堆的模块
- mtl包是Haskell Platform自带的，所以你可能已经有了
- 为了检查你有没有，键入在命令行`ghc-pkg list`
- 这将会展示你已经安装了哪些Haskell包，其中一个应该是mtl，之后是一个版本号

</div>
<h2 id="485D46DB">Writer?我根本不知道她！</h2>
<div class="sheet-wrap"><div class="sheet-caption">Writer的用途</div>


- 我们已经用Maybe单子、list单子和IO单子装填了我们的枪
- 现在让我们也把Writer单子装进去，然后看看我们在开火的时候会发生什么吧

- Maybe是用于带有额外失败上下文的值，list是用于非确定性计算，而Writer单子是用于一些值，附带有另外的值作为一种日志值
- Writer允许我们做计算的同时，确保所有的日志值组合为一个日志值，然后附加到结果

</div>
<div class="sheet-wrap"><div class="sheet-caption">使用示例</div>


例如，我们可能想要用能够解释发生了什么的字符串来装备我们的值，可能是出于debug目的
- 考虑一个函数，它从帮派里获取数个土匪，然后告诉我们那是不是个大帮派
- 这是个很简单的函数
  ``` Haskell
  isBigGang :: Int -> Bool
  isBigGang x = x > 9
  ```
- 现在，要是不是只给我们一个True或者False值，我们想要让它也返回一个长字符串，告诉我们它做了什么，如何？
- 我们只要创建那个字符串然后让它跟随我们的Bool返回：
  ``` Haskell
  isBigGang :: Int -> (Bool, String)
  isBigGang x = (x > 9, "Compared gang size to 9.")
  ```
- 所以现在，不是只返回一个Bool值，我们返回一个元组，其中元组的第一个成员是实际值，第二个成员是那个值伴有的字符串
- 我们的值现在添加了一些额外的上下文，让我们试试看
  ``` Haskell
  ghci> isBigGang 3
  ...
  ```
- 目前为止很好
- 那如果我们已经有了一个值，它附带有一个日志字符串，比如说`(3,"Smallish gang.")`，我们想要把它喂入`isBigGang`，又如何呢？
  - 看起来，又一次，我们面对这个问题：如果我们有一个函数获取普通的值并且返回一个带上下文的值，我们如何从上下文中获取值并且把它喂入函数？
  - 当我们探索Maybe单子，我们创建了一个函数applyMaybe，它获取`Maybe a`值以及一个类型`a -> Maybe b`函数，并且把`Maybe a`值喂入函数，即使函数获取一个普通值，而不是`Maybe a`
  - 它通过考虑`Maybe a`值的一个上下文来做到这个，也就是，值只是一个普通值，因为applyMaybe（后来变成>>=）关心检查它是Nothing还是Just值
- 以同样的方式（in the same vein）
  - 让我们创造一个函数，它获取一个带日志的值，也就是，一个`(a,String)`值和一个类型`a -> (b,String)`的函数
  - 并且把值喂入函数
  - 我们将会称之为`applyLog`
  - 但是因为`(a,String)`值不承载可能失败的上下文，而是附加日志值的上下文，`applyLog`将会确保原值的日志不会丢失，而是与该结果值的日志结合到一起
- 这是一个`applyLog`的实现
  ``` Haskell
  applyLog :: (a,String) -> (a -> (b,String)) -> (b,String)
  applyLog (x,log) f = let (y,newLog) = f x in (y,log ++ newLog)
  ```
- 当我们有一个带上下文的值，我们想要把它喂入函数，我们通常尝试把实际值从上下文分离出来，然后尝试把函数应用到值，然后看到上下文被处理了
  - 在Maybe单子中，我们检查值是否是`Just x`，如果它是，我们获取x然后把它应用到函数
  - 在这个例子中，很容易发现真实的值，因为我们做处理一对，其中一个成员是值，另一个是日志
  - 所以首先我们只需要获取值x，然后我们把函数f应用于它
  - 我们获取一对`(u,newLog)`，其中y是新的结果，newLog是新的日志
  - 但是如果我们把它作为结果返回，旧的日志不会包含到结果中，所以我们返回一对`(y,log ++ newLog)`
  - 我们使用`++`来把新的日志追加到旧的上面
- 这就是`applyLog`测试 \
  *代码略*
- 更多的测试 \
...
<div class="sheet-wrap"><div class="sheet-caption">回忆(->)r作为函子</div>


有关应用函子的章节中，我们看到函数类型，`(->) r`是函子的一个实例
- 把一个函数`f`应用于函数`g`将会产生一个函数
  - 其和`g`获取的东西一样，把`g`应用于它，然后再把结果应用于`f`
- 基本上，我们正在创造一种新的函数
  - 它像`g`
  - 只不过，在返回结果之前，`f`也被应用到那个结果
- 例如
  ``` Haskell
  ghci> let f = (*5)
  ghci> let g = (+3)
  ...
  55
  ```
- 我们也看到函数是应用函子，它们允许我们操作函数的最终结果，就好像我们已经获得了它们的结果一样，例如
  ``` Haskell
  ghci> let f = (+) <$> (*2) <*> (+10)
  ghci> f 3
  19
  ```
- 表达式`(+) <$> (*2) <*> (+10)`创造一个函数
  - 它获取一个数，把那个数给到`(*2)`和`(+10)`
  - 然后把结果加在一起

</div>
<div class="sheet-wrap"><div class="sheet-caption">函数类型(->)r是单子</div>


函数类型`(->) r`不只是一个函子，和一个应用函子，它也是一个单子
- 和我们到目前为止见到的其它单子值一样，函数也可以看作带有上下文的值
- 函数的上下文是，值还并没有出来，但是我们必须把那个函数应用到某些东西上，来获得它的结果值

因为我们已经熟悉了函数如何作为函子和应用函子运作，我们深入看看它们的`Monad`实例看起来如何吧
- 它位于`Control.Monad`，看起来像这样
  ``` Haskell
  instance Monad ((->) r) where
    return x = \_ -> x
    h >>= f = \w -> f (h w) w
  ```
  - 我们已经看到了`pure`对于函数是如何实现的，而`return`很大程度上和`pure`是一回事
  - `>>=`的实现看起来有点令人费解，实际上一点也不费解
    - 当我们使用`>>=`来把函子值喂入一个函数，结果总是函子值
    - 在这个例子中，当我们把一个函数喂入另一个函数，结果也是一个函数，所以起手式就是lambda
    - 目前所有`>>=`的实现总是把结果从单子值分离出来，然后把函数`f`应用到那个结果
    - 这里也是一样的
    - 为了从函数中获取结果，我们必须把它应用到某些东西上，这就是为什么我们要在这里`(h w)`，就是为了从函数中获得结果，然后把`f`应用到那上面
    - `f`返回一个单子值，它在我们的情况里面是个函数，所以我们也给它应用`w`

</div>
<div class="sheet-wrap"><div class="sheet-caption">示例</div>


带着例子，我们可以看到这真的是一个很简单的单子，这是一个使用这个单子的do表达式
``` haskell
...
addStuff = do
  ...
  return (a+b)
```
- 这和我们之前写的应用函子表达式是同一回事
- 只是现在它依靠函数作为函子
- 一个do表达式结果总是一个单子值，这个也不例外
- 这个函子值的结果是一个函数

reader单子
- `(*2)`和`(+10)`都被应用到数字3上
- `(a+b)`也是，但是它忽略了数字3,并且总是把`a+b`表示为结果
- 因此，这个函数单子也被成为reader单子
- 所有的函数都从共同的来源读入
- 为了甚至更好地演示，我们可以像这样重写`addStuff`
  ``` Haskell
  addStuff :: Int -> Int
  addStuff x = let
    a = (*2) x
    ...
    in a+b
  ```
- 我们看到reader单子允许我们把函数当作带有上下文的值
- 我们可以假装好像我们已经知道了函数会返回什么东西
- 它通过把函数固粘到一起，变成一个函数，然后把那个函数的参数交给固粘出它的所有的函数
- 所以，如果
  - 我们有很多个函数，都只是缺少一个参数
  - 它们最终被应用到同一个东西
  我们可以
  - 使用reader单子，以某种方式把它们未来的结果提取出来
  - 然后`>>=`实现将会确保正确运行出来
